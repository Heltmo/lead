...
Workflow queue controls now show a CRM-lite seller board with columns for Call now, Follow-up due, Interested, and Done / rejected. Queue presets for Call now, Needs verification, Follow-ups, and Interested leads still work with not-contacted, due follow-up, interested, and contacted/not-contacted filters. The default list sort is Call queue first. It prioritizes due follow-ups, interested leads, new phone-ready leads, exact-location leads, and useful org.nr context before lower-action items. Phone numbers render as `tel:` links so a seller can click from the browser when their device supports it, but no call is placed by the app itself. Each lead card also shows a short `Next:` queue action so the seller can scan the list without opening every card. Each workflow save appends a small local activity timeline, and exports include full call-list CSV views for all leads, today call queue, not-contacted leads, due follow-ups, and interested leads.

The lead detail and Today call queue also include one-click workflow actions for common manual outcomes: `Mark called`, `No answer`, `Interested`, `Not relevant`, `Follow up tomorrow`, and `Follow up next week`. In the seller board, follow-up shortcuts appear as compact `Tomorrow` and `Next week` actions. Follow-up rows are labelled as overdue, due today, or future follow-up so the seller knows what must be handled first. These buttons only update local workflow state and the activity timeline. They do not call, send email, create calendar events, or automate outreach. Queue actions advance the selected lead toward the next call-ready item after saving, and `Inspect`/lead-card selection scrolls the lead detail into view. This keeps the demo close to a call-ready sales desk while still avoiding CRM/telephony/email integration.
