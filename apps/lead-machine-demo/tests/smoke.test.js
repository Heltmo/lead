...
async function main() {
  ...
  assert(normalizeWorkflow({ outcome: 'nei' }).queue === 'not_relevant', 'nei outcome should move to no queue')
  ...
  response = await post(port, '/api/runs', { query: 'rørlegger i Kristiansand; rm -rf /', provider: 'google-places', searchScope: 'strict', sellerIntent: 'telecom', sellerProfile: { territory: 'Kristiansand', goodCustomer: 'local trade phone-ready', disqualifiers: 'restaurant' }, enrichCompanyProfile: false })
  ...
  assert(runnerArgs.sellerProfile.goodCustomer === 'local trade phone-ready', 'seller profile should flow into the runner options')
  ...
  assert(response.body.summary.sellerProfile.goodCustomer === 'local trade phone-ready', 'completed run should return seller profile in summary')
  assert(response.body.leadPacks[0].sellerFit.sellerIntent === 'telecom', 'completed run should attach seller fit to lead packs')
  assert(response.body.leadPacks[0].sellerFit.sellerProfile.territory === 'Kristiansand', 'completed run should attach seller profile to lead packs')
  assert(response.body.leadPacks[0].sellerFit.fitReasons.includes('matcher selgergeografi: kristiansand'), 'seller territory should affect lead fit reasons')
  assert(response.body.leadPacks[0].sellerFit.fitReasons.includes('matcher god kunde-hint: local'), 'good-customer setup should affect lead fit reasons')
  ...
  assert(response.body.leadPacks[0].queueQuality.recommendedQueue === 'call_now', 'phone-ready lead with a non-weak website verdict should be callable')
  ...
  assert(response.body.savedSearches[0].sellerProfile.goodCustomer === 'local trade phone-ready', 'saved searches should persist seller profile setup')
  ...
  assert(workflowCsv.body.includes('ring igjen'), 'CSV should include workflow next action from quick action')
  ...
  const deepResponse = await post(port, '/api/deep-qualify', { query: 'rørlegger i Kristiansand', lead: response.body.leadPacks[0], sellerProfile: { territory: 'Kristiansand', goodCustomer: 'VVS trade', disqualifiers: 'restaurant' }, enrichCompanyProfile: true })
  ...
  assert(deepQualifierArgs.sellerProfile.goodCustomer === 'VVS trade', 'deep qualification should receive the current seller profile')
  assert(deepResponse.body.leadPack.callPriority === 'high', 'deep qualification should return one updated lead pack')
  assert(deepResponse.body.leadPack.sellerFit.sellerProfile.goodCustomer === 'VVS trade', 'deep qualification should return seller profile fit context')
  ...
  assert(lower.includes('i dag / kommandosenter'), 'UI should label the Command Center surface')
  assert(lower.includes('neste: ring') && lower.includes('start ringeøkt') && lower.includes('i dag / kommandosenter'), 'Command center should be a single next-move line with a call-session start')
  ...
  assert(lower.includes('mobile-active-bar') && lower.includes('data-mobile-edit-search') && lower.includes('mobile-search-open'), 'Mobile active search header should collapse the selling viewport after results')
  assert(lower.includes('mobile-session-start') && lower.includes('callfocusstartdisabledattr') && lower.includes('startleadid'), 'Mobile should make Ringeøkt first-class and start on the selected eligible lead')
  assert(lower.includes('mobile-decision-card') && lower.includes('mobil beslutningskort') && lower.includes('mobile-decision-more'), 'Mobile selected lead should render a compact decision card with expandable details')
  assert(lower.includes('data-mobile-note-toggle') && lower.includes('data-mobile-note-input') && lower.includes('data-mobile-save-note'), 'Mobile call bar should expose an expandable note drawer')
  assert(lower.includes('data-queue-select') && lower.includes('queue-select-mobile'), 'Mobile should offer the work queues as a compact dropdown')
  assert(lower.includes('data-mobile-more') && lower.includes('mobile-more-menu'), 'Mobile call bar should keep secondary actions behind an overflow menu')
  assert(appJs.includes('appendSellerNote') && appJs.includes('clearMobileNoteDraftFor') && appJs.includes('state.mobileQueueDone = { queue: state.selectedQueue, action }'), 'Mobile outcome saves should append notes and show a queue-finished state when no next lead exists')
  ...
  assert(lower.includes('data-workflow-action="sale"') && lower.includes('salg') && lower.includes('data-workflow-action="not_relevant"') && lower.includes('nei'), 'Selected lead should expose concrete sales outcome logging')
  ...
  assert(!lower.includes('ownerinput') && lower.includes('leadmachinebetaowner'), 'Beta tester owner should be persisted without a visible header input')
  ...
  assert(lower.includes('mer info') && lower.includes('support-info-panel'), 'Supporting lead info should live in one consolidated section')
  ...
  assert(appJs.indexOf('workflowPanel(lead)') < appJs.indexOf('supportInfoPanel(lead, command)'), 'Supporting lead info should sit below the seller action path')
  ...
  assert(lower.includes('bekreftet org.nr') || lower.includes('bekreftet org.nr'), 'UI should support confirmed org number filtering/sorting')
  ...
  assert(!lower.includes('professionselect') && !lower.includes('locationinput'), 'The old structured profession/location fields should stay removed')
  ...
  assert(lower.includes('alle byer'), 'UI should expose Norway sweep city filter chips')
  ...
  assert(lower.includes('id="searchscope" type="hidden" value="regional"'), 'UI should keep search scope as a hidden regional default')
  assert(!lower.includes('<span>område</span><select id="searchscope"'), 'UI should not expose a geography dropdown in the search header')
  ...
  assert(lower.includes('raskt søk'), 'Fast mode UI should show fast scan state')
  assert(lower.includes('trenger du sterkere bevis?'), 'Fast leads should show optional Verify & Enrich state')
  assert(lower.includes('verifiser firma'), 'Fast leads should expose selected-lead Verify & Enrich action')
  assert(lower.includes('verifiser firma (hosted)') && lower.includes('lett firma-/kontakt-/kildeoppfrisking') && lower.includes('full lokal sjekk er ikke kjørt'), 'UI should distinguish hosted Verify & Enrich as a lightweight refresh')
  ...
  assert(lower.includes('nettsidesjekk'), 'Verify & Enrich should show digital presence module')
  assert(lower.includes('brreg verification'), 'Verify & Enrich should show Brreg verification module')
  assert(lower.includes('økonomi / proff'), 'Verify & Enrich should show economy/Proff module status')
  assert(lower.includes('sosiale/kildesignaler'), 'Verify & Enrich should show social/source module status')
  assert(lower.includes('beslutningstakere'), 'Verify & Enrich should show decision maker module status')
  assert(lower.includes('nylig aktivitet'), 'Verify & Enrich should show recent activity module status')
  assert(lower.includes('selgermatch-sammendrag'), 'Verify & Enrich should show seller fit module status')
  ...
  assert(lower.includes('tilleggsmoduler') && lower.includes('verifiser firma prøver brreg på nytt'), 'Verify & Enrich support modules should remain available inside Mer info')
  assert(lower.includes('call brief'), 'UI should expose a neutral call brief section')
  assert(lower.includes('kontaktbarhet og firmakontekst avgjør salgsverdien'), 'UI should define generic sales usability separately from website pain')
  ...
  assert(lower.includes('beste første kontakt'), 'UI should expose best first contact')
  assert(lower.includes('firmamatch'), 'UI should expose company fit')
  assert(lower.includes('verification'), 'UI should expose verification status')
  assert(lower.includes('hovedrisiko'), 'UI should expose main risk')
  assert(lower.includes('neste handling'), 'UI should expose next action')
  assert(lower.includes('kildetrygghet'), 'UI should expose source confidence')
  assert(lower.includes('identitet uavklart'), 'UI should avoid making temporary Brreg failure the lead headline')
  assert(lower.includes('brreg ikke bekreftet'), 'UI should expose Brreg not-confirmed source status')
  assert(lower.includes('verifiser firma for nytt forsøk'), 'UI should frame Brreg gaps as selected-lead retry work')
  assert(lower.includes('brreg fant ingen bekreftet identitet'), 'UI should explain no-match identity state')
  assert(lower.includes('websitevalue'), 'UI should normalize website values before rendering')
  assert(lower.includes('selgerkontekst'), 'UI should expose seller context section')
  assert(lower.includes('seller-desk-v2'), 'UI should include seller desk V2 cards')
  assert(lower.includes('bedriftstype'), 'Call brief should expose business type')
  assert(lower.includes('bevis og sjekker'), 'UI should combine proof and verification into a decision card')
  assert(lower.includes('bevis og trygghet'), 'UI should expose Proof & confidence section')
  ...
  assert(lower.includes('firmaidentitet'), 'UI should expose company identity context')
  assert(lower.includes('contactability'), 'UI should expose contactability context')
  assert(lower.includes('markedsbevis'), 'UI should keep market proof in details')
  assert(lower.includes('qualification'), 'UI should expose qualification card')
  assert(lower.includes('salgssignaler'), 'UI should expose sales signals card')
  assert(lower.includes('risiko / verifiser'), 'UI should expose risk/verify card')
  ...
  assert(lower.includes('kvalifisering og verifiseringsdetaljer'), 'UI should collapse secondary qualification details')
  assert(lower.includes('eksportstatus'), 'UI should expose export readiness state')
  assert(lower.includes('hvorfor vurdere?'), 'UI should collapse supporting reasons behind a seller-focused details row')
  assert(lower.includes('sources'), 'UI should include collapsible source intelligence')
  assert(lower.includes('rådata'), 'UI should include collapsible raw lead data')
  ...
  assert(lower.includes('treffkvalitet'), 'UI should expose discovery quality source card')
  assert(lower.includes('treffsikkerhet'), 'UI should expose discovery confidence')
  assert(lower.includes('brreg firmaprofil'), 'UI should expose Brreg firmaprofil panel')
  assert(lower.includes('bekreftet org.nr'), 'UI should expose confirmed org number field')
  assert(lower.includes('kandidat org.nr'), 'UI should expose candidate org number field')
  assert(lower.includes('kandidat juridisk navn'), 'UI should expose candidate legal name field')
  assert(lower.includes('registrert adresse'), 'UI should expose registered address field')
  ...
  assert(lower.includes('kildestrategi'), 'UI should expose identity/presence source strategy')
  assert(lower.includes('identitetskilde'), 'UI should expose identity source')
  assert(lower.includes('synlighetskilde'), 'UI should expose presence source')
  assert(lower.includes('synlighet-først fallback'), 'UI should explain fallback when Brreg is not confirmed')
  ...
  assert(lower.includes('oppfølging forfalt'), 'UI should filter follow-up due leads')
  ...
  assert(lower.includes('aktivitetslogg'), 'UI should show visible local activity log')
  ...
  assert(lower.includes('ikke kontaktet'), 'UI should explain why new phone leads are in the queue')
  assert(lower.includes('oppfølging forfalt'), 'UI should include follow-up due leads in the queue')
  assert(lower.includes('inspiser'), 'UI should allow inspecting a queued lead')
  ...
  assert(lower.includes('salg'), 'UI should include a clear Sale quick action')
  ...
  assert(lower.includes('nei'), 'UI should include No quick action')
  assert(appJs.includes("['sale', 'interested', 'no_answer', 'not_relevant']"), 'Queue quick actions should stay focused on concrete seller outcomes')
  ...
  assert(appJs.includes("response: 'meeting_booked'") && appJs.includes("outcome: 'sale'"), 'Sale quick action should persist a distinct positive sale outcome')
  ...
  assert(lower.includes('lead machine'), 'Header should keep the Lead Machine brand visible')
  assert(!lower.includes('finn leads · ring · logg utfall · følg opp'), 'Header should remove the top-right workflow box')
  assert(!lower.includes('søk fritt i hele norge'), 'Search panel should not show helper text under the input')
  assert(!lower.includes('hva selger du?'), 'Seller category choice should stay removed until categories earn their place')
  assert(!lower.includes('selgeroppsett'), 'UI should not show a seller setup strip that distracts from search')
  assert(!lower.includes('seller-setup-panel') && !lower.includes('seller-setup-collapse'), 'UI should remove the compact seller setup panel')
  assert(!lower.includes('sellerterritory') && !lower.includes('idealcustomer') && !lower.includes('leadmachinesellersetup'), 'UI should not collect or persist hidden seller setup from the search header')
  assert(lower.includes('sellerprofile'), 'UI should still submit a neutral seller profile shape')
  assert(lower.includes("sellerintent: 'web_it'"), 'UI should always run searches with the website-sales intent')
  assert(lower.includes('website-sales-panel'), 'Selected lead should show the website sales verdict panel')
  assert(lower.includes('ingen nettside funnet'), 'Missing website should be presented as a positive website-sales signal')
  ...
  assert(lower.includes('data-run-sales-angles') && lower.includes('/api/sales-angles'), 'Selected lead should offer sales angle research')
  assert(lower.includes('finn salgsvinkler'), 'Sales angle research should use Norwegian seller-facing copy')
  assert(!lower.includes('data-run-website-audit') && !lower.includes('kjør nettsidesjekk'), 'Selected lead should only show one manual research button')
  assert(lower.includes('kort notat etter samtalen'), 'Selected lead should keep one visible note field in the seller flow')
  assert(lower.includes('latestleadnote'), 'Lead cards and call focus should surface the latest note')
  assert(lower.includes('data-start-call-focus'), 'UI should offer a start call-focus session button')
  assert(lower.includes('data-call-focus-outcome') && lower.includes('call-focus-action-dock'), 'Call focus mode should log outcomes one lead at a time with reachable mobile actions')
  ...
  assert(lower.includes('dailycheckin') && lower.includes('dagens innsjekk'), 'Main seller desk should include a daily check-in sidebar')
  assert(lower.includes('data-daily-checkin') && lower.includes('leadmachinedailycheckin'), 'Daily check-in should persist todays seller check locally')
  assert(lower.includes('latestcalledcustomers') && lower.includes('siste ringt'), 'Daily sidebar should show the last called customers')
  assert(lower.includes('dailycallcategory') && lower.includes('daily-log-category') && lower.includes('slice(0, 10)'), 'Daily call log should mark categories for the 10 latest called customers')
  ...
  assert(lower.includes('selgermatch'), 'UI should expose seller fit output')
  ...
  assert(!lower.includes('data-query-example'), 'UI should keep the search panel free of example chips')
  assert(lower.includes('searchsuggestions'), 'UI should expose autocomplete suggestions without separate required fields')
  assert(!lower.includes('summarypanel'), 'Noisy run-metrics row should stay removed from the seller desk')
  ...
  assert(!lower.includes('lagrede søk'), 'Saved search management UI should stay removed from the seller desk')
  assert(!lower.includes('data-saved-search'), 'Saved search load/pin/rename controls should stay removed from the seller desk')
  ...
  assert(lower.includes('rendercitychips'), 'UI should render sweep city chips inside the filter controls')
  ...
  assert(lower.includes('klar til å ringe'), 'UI should expose Ready to call state')
  assert(lower.includes('verifiser først'), 'UI should expose Verify first state')
  assert(lower.includes('trenger kontakt'), 'UI should expose Needs contact state')
  assert(lower.includes('med telefon'), 'Command center should show phone-ready counts')
  assert(lower.includes('slice(0, 8)'), 'Activity timeline should show up to eight recent entries')
  assert(lower.includes('support-info-panel'), 'Lead detail should use one support info section instead of many lower accordions')
  assert(lower.includes('hvorfor vurdere?'), 'Lead detail should keep inspect rationale available')
  assert(lower.includes('data-queue-preset'), 'UI should expose call queue preset buttons')
  assert(lower.includes('filtrer / sorter leads'), 'Advanced lead filters should be collapsed behind a filter/sort control')
  assert(lower.includes('kø først'), 'UI should support call-queue-first sorting')
  assert(lower.includes('beste selgermatch'), 'UI should support seller-fit-first sorting')
