...
async function main() {
  ...
  assert(lower.includes('pipeline-board'), 'UI should render the seller queue as a CRM-lite board')
  assert(lower.includes('workflowcolumns'), 'UI should group leads into seller workflow columns')
  assert(lower.includes('call now'), 'Workflow board should include a Call now column')
  assert(lower.includes('done / rejected'), 'Workflow board should include a Done / rejected column')
