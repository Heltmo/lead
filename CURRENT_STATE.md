...
- AI-nettsidesjekk (core/website-audit): a manual per-lead button that fetches the lead's homepage, extracts signals (generator, viewport, forms, years) and asks Claude for a short Norwegian verdict - estimated era, outdated yes/no, top issues, what's missing. The result feeds the website sales verdict (confirmed-weak site => strong lead, confirmed-modern => review) and is auto-saved as a card note. Requires LEAD_MACHINE_ANTHROPIC_API_KEY in .env; never runs in bulk; observations only, no outreach scripts. Local server only (not hosted beta).
