...
- Decluttered middle band: the run-metrics row, the standalone testdata strip and the command-center tiles/signals are removed. The command center is one line - "Neste: ring X" with Åpne lead and Start ringeøkt - city chips live inside the lead filter controls, and workspace export moved into the export panel.
- Simplified search header: free-text search first, with the visible seller setup strip removed so sellers only see search, queues, calls, notes, and exports. Saved searches persist server-side (command center and export) but the management panel is removed from the seller desk.
...
- AI-nettsidesjekk (core/website-audit): a manual per-lead button that fetches the lead's homepage, extracts signals (generator, viewport, forms, years) and asks OpenAI GPT-5.5 for a short Norwegian verdict - estimated era, outdated yes/no, top issues, what's missing. The result feeds the website sales verdict (confirmed-weak site => strong lead, confirmed-modern => review) and is auto-saved as a card note. Requires LEAD_MACHINE_OPENAI_API_KEY in .env; never runs in bulk; observations only, no outreach scripts. Runs locally and in hosted beta when the key is configured.
