...

{
  ...
  assert(result.fitReasons.includes('direkte telefon tilgjengelig'), 'phone should be a fit reason')
...

{
  ...
  assert(result.riskReasons.includes('ingen direkte telefon funnet'), 'missing phone should be a risk')
...

{
  ...
  assert(result.importantSignals.some((item) => item.includes('digital synlighet')), 'web_it should prioritize digital presence')
  assert(result.fitReasons.includes('digitalt synlighetsproblem kan være relevant'), 'web_it should treat digital issues as relevant')
}

{
  const result = evaluateSellerFit(lead(), 'telecom')
  assert(result.importantSignals.some((item) => item.includes('telefon, lokasjoner')), 'telecom should prioritize phone/company structure')
}

{
  const result = evaluateSellerFit(lead(), 'accounting')
  assert(result.importantSignals.some((item) => item.includes('verifisert juridisk identitet')), 'accounting should prioritize legal identity')
}

{
  ...
  assert(result.fitReasons.includes('matcher selgergeografi: oslo'), 'seller geography should affect fit reasons')
  assert(result.fitReasons.includes('matcher god kunde-hint: vvs'), 'good-customer hint should affect fit reasons')
  ...
  assert(disqualified.riskReasons.includes('matcher diskvalifisering: restaurant'), 'disqualifiers should create risk reasons')
