...
function entity({ organisasjonsnummer, navn, poststed = 'OSLO', kommune = 'Oslo', telefon, mobil, hjemmeside }) {
  return {
    organisasjonsnummer,
    navn,
    organisasjonsform: { kode: 'AS', beskrivelse: 'Aksjeselskap' },
    forretningsadresse: { adresse: ['Testveien 1'], postnummer: '1600', poststed, kommune, kommunenummer: '3107', landkode: 'NO', land: 'Norge' },
    ...
    mobil,
    hjemmeside,
    _links: { self: { href: `https://data.brreg.no/enhetsregisteret/api/enheter/${organisasjonsnummer}` } },
  }
}

function mockFetch({ enheter = [], underenheter = [] }) {
  return async (url) => ({
    ok: true,
    status: 200,
    async json() {
